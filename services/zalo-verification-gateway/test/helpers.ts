...
export function signedHeaders(body: Buffer, options: {
  keyId?: string; keyHex?: string; timestamp?: string; nonce?: string;
  ...
  const canonical = buildCanonicalRequest({
    method: options.method ?? "POST", path: options.path ?? "/v1/zalo/verify",
    timestamp, nonce, requestId, bodySha256,
  });
  return {
    "content-type": "application/json", "x-key-id": keyId, "x-timestamp": timestamp,
    "x-nonce": nonce, "x-request-id": requestId, "x-body-sha256": bodySha256,
